...
class PuzzleSolverScreen(tkinter.Frame):
    def __init__(self, parent, controller):
        ...
        self.label_1 = customtkinter.CTkLabel(self.main_frame, text="Select The Puzzle:", font=customtkinter.CTkFont(size=24, weight="normal"))
    ...
    def get_rows_cols(self):
        # Retrieve and validate rows and cols from entries
        try:
            rows = int(self.rows_button.get())
            cols = int(self.cols_button.get())
            print(rows)
            print(cols)
            print(self.puzzle_image_path)
            if(self.puzzle_image_path == None):
                tkinter.messagebox.showerror("Error", "Enter Puzzle")
            else:
                self.puzzle_image_path = self.puzzle_image_path.replace("/", "\\")
                solve_puzzle(self.puzzle_image_path, rows, cols)

        except ValueError:
            tkinter.messagebox.showerror("Error", "Rows and Columns must be integers")
            return None, None
