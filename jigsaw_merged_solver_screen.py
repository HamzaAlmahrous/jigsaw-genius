...
x_folder_path = os.path.join(parent_dir, 'jigsaw-genius\\solvers\\part_2_merged\\')
sys.path.append(x_folder_path)

from part_2_merged_solve import merged_solve

class JigsawMergedScreen(tkinter.Frame):
    ...
    def solve(self):
        if(self.hint_image_path == None or self.jigsaw_image_path == None):
            messagebox.showerror("Error", "Enter Jigsaw and Hint")
        else:
            self.jigsaw_image_path = self.jigsaw_image_path.replace("/", "\\")
            self.hint_image_path = self.hint_image_path.replace("/", "\\")
            merged_solve(self.jigsaw_image_path, self.hint_image_path)